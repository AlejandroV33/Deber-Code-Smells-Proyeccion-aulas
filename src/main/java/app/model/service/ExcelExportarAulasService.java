    ...
    public void exportarVisor(List<AulaOcupacion> aulas, File file) throws Exception {
        try (Workbook wb = new XSSFWorkbook()) {
            ...
            for (int a = 0; a < aulas.size(); a++) {
                ...
                CellStyle currentAulaStyle = isShaded ? aulaShadedStyle : aulaNormalStyle;

                int startRow = rowIndex;
                int endRow = startRow + 12; // 13 horas (7 a 20)

                // NUEVO: Escaneo previo para descubrir qué horas están realmente ocupadas
                boolean[] horasOcupadas = new boolean[13];
                for (AulaOcupacion.Bloque b : aula.getBloques()) {
                    for (int h = b.inicio; h < b.fin; h++) {
                        if (h >= 7 && h < 20) {
                            horasOcupadas[h - 7] = true;
                        }
                    }
                }

                // Crear las 13 filas de horas
                for (int h = 0; h < 13; h++) {
                    Row r = sheet.createRow(startRow + h);

                    // LÓGICA DE ALTURA DINÁMICA
                    if (horasOcupadas[h]) {
                        r.setHeightInPoints(45); // Espacio amplio para bloques con clase
                    } else {
                        r.setHeightInPoints(18); // Espacio mínimo para horas muertas
                    }

                    for(int c = 0; c < 8; c++){
                        r.createCell(c).setCellStyle(emptyStyle);
                    }

                    Cell cHora = r.getCell(1);
                    int horaIn = 7 + h;
                    cHora.setCellValue(horaIn + "-" + (horaIn + 1));
                    cHora.setCellStyle(horaStyle);
                }
                ...
                for (AulaOcupacion.Bloque b : aula.getBloques()) {
                    ...
                    cBlock.setCellStyle(bloqueStyle);
                }

                // Fila separadora tenue
                Row sepRow = sheet.createRow(endRow + 1);
                sepRow.setHeightInPoints(10);
                for (int c = 0; c < 8; c++) {
                    sepRow.createCell(c).setCellStyle(separadorStyle);
                }
