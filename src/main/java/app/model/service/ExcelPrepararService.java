...
public class ExcelPrepararService {
    ...
    public Workbook preparaExcelMemoria(File inputFile, Consumer<String> logger) throws Exception {
        ...
        maxCols = Math.max(maxCols, colSemIdx + 1);
        ...
        // ==========================================
        // LÓGICA DE UNIFICACIÓN INTELIGENTE (CON TILDES Y Ñ)
        // ==========================================
        ...
        // Ordenamos del más largo al más corto para que los incompletos se aniden en los completos
        ...
        // ==========================================
        // REPARACIÓN DEFINITIVA DE FECHAS (ej. 11-ene -> 11-13)
        // ==========================================
        logger.accept(">> corrigiendo auto-formatos de fecha en las horas de clase...");
        encabezados.clear();
        for (int c = 0; c < maxCols; c++) encabezados.add(getCellString(headerRow.getCell(c)));
        encabezadosNorm = encabezados.stream().map(ExcelPrepararService::normalizarParaBusqueda).collect(Collectors.toList());
        ...
        // Candado de Excel para que las horas queden como texto puro
        ...
        // ==========================================
        // DETECCIÓN DE CHOQUES (Ahora usando la data 100% limpia)
        // ==========================================
        ...
        // Choques Profesor: Agrupados usando Normalización Estricta
        ...
        logger.accept(">> generando hojas de conflictos en el archivo...");
...
public class ExcelPrepararService {
    ...
    // Retorna el Workbook procesado en memoria. Lanza excepcion si algo falla.
    public Workbook preparaExcelMemoria(File inputFile, Consumer<String> logger) throws Exception {
        logger.accept(">> cargando archivo excel: " + inputFile.getName());
        ...
        List<String> encabezadosNorm = encabezados.stream().map(ExcelPrepararService::normalizarEncabezado).collect(Collectors.toList());
        ...
        logger.accept(">> insertando columna 'Semestre' y normalizando textos...");
        int colSemIdx = colMatIdx + 1;
        insertarColumna(ws, colSemIdx, maxCols);
        ...
        CellStyle estiloSem = wb.createCellStyle();
        XSSFFont fuente = wb.createFont();
        fuente.setFontName("Verdana"); fuente.setFontHeightInPoints((short)12);
        estiloSem.setFont(fuente);
        estiloSem.setBorderTop(BorderStyle.THIN); estiloSem.setBorderBottom(BorderStyle.THIN);
        estiloSem.setBorderLeft(BorderStyle.THIN); estiloSem.setBorderRight(BorderStyle.THIN);
        headerRow.getCell(colSemIdx).setCellStyle(estiloSem);
        ...
        Set<String> nombresCompletos = new HashSet<>();
        for (int r = 1; r <= lastRow; r++) {
            Row row = ws.getRow(r);
            if (row == null) continue;
            String prof = normalizarTexto(getCellString(row.getCell(colProfIdx)));
            if (prof != null && prof.split("\\s+").length >= 3) nombresCompletos.add(prof);
        }
        ...
        for (int r = 1; r <= lastRow; r++) {
            ...
            Cell cProf = row.getCell(colProfIdx);
            if (cProf != null && cProf.getCellType() != CellType.BLANK) {
                cProf.setCellValue(obtenerNombreEquivalente(normalizarTexto(getCellString(cProf)), nombresCompletos));
            }
            Cell cMat = row.getCell(colMatIdx);
            if (cMat != null && cMat.getCellType() != CellType.BLANK) {
                cMat.setCellValue(normalizarTexto(getCellString(cMat)));
            }
        }

        logger.accept(">> analizando posibles choques de horarios...");
        // Re-leer encabezados por el cambio
        encabezados.clear();
        for (int c = 0; c < maxCols; c++) encabezados.add(getCellString(headerRow.getCell(c)));
        encabezadosNorm = encabezados.stream().map(ExcelPrepararService::normalizarEncabezado).collect(Collectors.toList());
        ...
        for (List<String> fila : filasDatos) {
            String prof = fila.get(colProf);
            if (!prof.trim().isEmpty()) profMap.computeIfAbsent(prof, k -> new ArrayList<>()).add(fila);
        }
        Set<List<String>> choquesProfesorSet = new HashSet<>();
        for (List<List<String>> grupo : profMap.values()) {
            for (int i = 0; i < grupo.size(); i++) {
        ...
        logger.accept(">> analisis completado con exito. listo para guardar.");
        return wb; // Retornamos el objeto para que el controlador lo guarde
    }

    // --- MÉTODOS AUXILIARES ORIGINALES (Ligeramente compactados) ---
    private static void crearHojaConFilas(Workbook wb, String nom, List<String> enc, Collection<List<String>> filas) {
        Sheet s = wb.createSheet(nom); Row h = s.createRow(0);
        for (int i = 0; i < enc.size(); i++) h.createCell(i).setCellValue(enc.get(i));
        List<List<String>> ord = filas.stream().sorted(Comparator.comparing(f -> String.join("|", f))).toList();
    ...
    private static String getCellString(Cell cell) {
        ...
        switch (cell.getCellType()) {
            ...
            case NUMERIC: return DateUtil.isCellDateFormatted(cell) ? cell.getDateCellValue().toString() : String.valueOf(cell.getNumericCellValue() % 1 == 0 ? (long)cell.getNumericCellValue() : cell.getNumericCellValue());
    ...
    private static String normalizarTexto(String t) { return t == null ? "" : Normalizer.normalize(t.trim().toUpperCase(), Normalizer.Form.NFKD).replaceAll("\\p{M}", ""); }
    private static String normalizarEncabezado(String t) { return normalizarTexto(t); }
    ...
    private static String obtenerNombreEquivalente(String n, Set<String> completos) {
        if (n == null || n.isEmpty()) return "";
        Set<String> pc = new HashSet<>(Arrays.asList(n.split("\\s+")));
        if (pc.size() <= 2) {
            for (String l : completos) if (new HashSet<>(Arrays.asList(l.split("\\s+"))).containsAll(pc)) return l;
        }
        return n;
    }
    ...
    private static Set<List<String>> encontrarChoquesSemestre(List<List<String>> filas, int cSem, int cMat, List<Integer> idxDias) {
        ...
        for (List<String> f : filas) {
            String sem = cSem < f.size() ? f.get(cSem) : "", mat = cMat < f.size() ? f.get(cMat) : "";
            if (sem != null && !sem.isEmpty() && mat != null && !mat.isEmpty()) semDict.computeIfAbsent(sem, k -> new HashMap<>()).computeIfAbsent(mat, k -> new ArrayList<>()).add(f);
        }
        Set<List<String>> choques = new HashSet<>();
        for (Map<String, List<List<String>>> materias : semDict.values()) {
            List<String> matList = new ArrayList<>(materias.keySet());
